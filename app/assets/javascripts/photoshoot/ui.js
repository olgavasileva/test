...
var psUI = function()
{
  ...
  function drawerUI(handle, parent, assetContainer){
        ...
        function bindEvents() {
            $(drawerHandle).click( function(e){ controller(e); });
        }
  ...
  function saveScene(logged_in){
    ...
    ret.done(function(scene){
        ...
        if (logged_in)
          ...
          var params = window.location.href.split("?")[1], paramString= "";
...
//  }
  ...
  function renderPackList(){
    ...
    $(elMenuId).html(template({sticker_packs: psUtils().getPacks()}));
  ...
  function renderPackDetails( pack ) {
    ...
    $(elMenuId).html(template({sticker_pack: pack}));
  ...
  this.share = function() {
    $("#thanku-dialog").dialog("close");
    renderTemplate("share_form", {"itemId": PS_SCENE_ID, "itemType": "scene", "shareUrl": FP_APP_DOMAIN + "/scenes/share/" + PS_SCENE_ID + ".json", "detailsUrl": PS_DETAILS_URL, "studioName": PS_STUDIO_NAME}, function(content) {
