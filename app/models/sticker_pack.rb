class StickerPack < ActiveRecord::Base
  ...
  def available_stickers
    Rails.cache.fetch("#{Rails.env}:#{self.id}:stickers".hash.to_s, :expires_in => (STICKER_CACHE_TTL ? STICKER_CACHE_TTL : 1.minutes)) do
      available_stickers = stickers.where.not(type: 'Background')
    end
  end

  def backgrounds
    Rails.cache.fetch("#{Rails.env}:#{self.id}:backgrounds".hash.to_s, :expires_in => (STICKER_CACHE_TTL ? STICKER_CACHE_TTL : 1.minutes)) do
