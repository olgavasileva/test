...
describe :instances do
  ...
  context "With no params" do
    ...
    it {expect(json['error_message']).to match /api_signature is missing/}
  end
  ...
  context "With all required params" do
    ...
    context "When the signature doesn't match" do
      ...
      it {expect(json['error_code']).to eq 1000}
      it {expect(json['error_message']).to match /Invalid API signature/}
      it {expect(json['instance_token']).to be_nil}
    end
    ...
    context "When the signature is OK" do
      ...
      it {expect(json['share_community_public']).to eq Setting.find_by_key('share_community_public').try(:value) }
      it {expect(json['share_community_private']).to eq Setting.find_by_key('share_community_private').try(:value) }
      ...
      context "When the signature is uppercase" do
        ...
        it {expect(json['instance_token']).not_to be_nil}
      end
      ...
      context "When the instance_token is not supplied" do
        ...
        it "should create an instance and return its token" do
          expect(Instance.find_by(uuid:json['instance_token'])).not_to be_nil
        end
        it {expect(json['api_domain']).to be_present}
        it {expect(json['background_images'].class).to eq Array}
        it {expect(json['background_images_retina'].class).to eq Array}
        it {expect(json['background_choice_images'].class).to eq Array}
        it {expect(json['background_choice_images_retina'].class).to eq Array}
        it {expect(json['background_order_choice_images'].class).to eq Array}
        it {expect(json['background_order_choice_images_retina'].class).to eq Array}
        ...
        context "When there are two enabled and one disabled key/value pairs in Settings" do
          ...
          it {expect(json['Superman']).to eq "Clark Kent"}
          it {expect(json['007']).to eq "James Bond"}
          it {expect(json['86']).to be_nil}
        end
        ...
        context "When there is one background image of each type (quetion, choice, order choice) and for standard and retina displays" do
          ...
          it {expect(json['background_images']).to eq ["DEVICE QUESTION URL"]}
          it {expect(json['background_images_retina']).to eq ["RETINA DEVICE QUESTION URL"]}
          it {expect(json['background_choice_images']).to eq ["DEVICE CHOICE URL"]}
          it {expect(json['background_choice_images_retina']).to eq ["RETINA DEVICE CHOICE URL"]}
          it {expect(json['background_order_choice_images']).to eq ["DEVICE ORDER URL"]}
          it {expect(json['background_order_choice_images_retina']).to eq ["RETINA DEVICE ORDER URL"]}
        end

        it "should set the app version on the instance" do
          expect(Instance.find_by(uuid:json['instance_token']).app_version).to eq app_version
        end
      end
        ...
        context "When the supplied instance_token is not nil" do
          ...
          context "When the instance already exists" do
            ...
            it "should use that instance and return the same token" do
              expect(Instance.find_by(uuid:json['instance_token']).id).to eq existing_instance.id
            end
            it "should increment the launch_count" do
              expect(Instance.find_by(uuid:json['instance_token']).launch_count).to eq 1
            end
            it "should set the app version on the instance" do
              expect(Instance.find_by(uuid:json['instance_token']).app_version).to eq app_version
            end
