...
describe Respondent do
  ...
  describe :demographic_required? do
    subject {respondent.reload.demographic_required?}
    ...
    context "When there is demographic data for this respondent that is 29 days old" do
      before {FactoryGirl.create :demographic, respondent: respondent, updated_at: (Date.current - 1.month)}

      it { expect(subject).to eq false}
    end

    context "When there is demographic data for this respondent that is 30 days old" do
      before {FactoryGirl.create :demographic, respondent: respondent, updated_at: (Date.current - 1.month - 1.day)}
